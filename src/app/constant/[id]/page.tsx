...
import useHandleDeleteConstant from "../../hooks/useHandleDeleteConstant";
...
const constant = ({ params }: Props) => {
    ...
    const { handleDelete, error } = useHandleDeleteConstant(dadosTemp, setDados);
        ...
        <Layout>
            <div className="cropsPage">
                ...
                <div className="list-constants">
                    ...
                    <Table data={dadosTemp} columns={columns} onDelete={(id) => handleDelete(id, 'constant')} />

                    <div className="header-list">
                        <div className="header-col-type">Tipo</div>
                        <div className="header-col-value">Valor</div>
                        <div className="header-col-reference">Referência</div>
                        <div className="header-col-climate">Clima</div>
                        <div className="header-col-biome">Bioma</div>
                        <div className="header-col-country">País</div>
                        <div className="header-col-irrigation">Irrigação</div>
                        <div className="header-col-cultivationSystem">Sistema de cultivo</div>
                        <div className="header-col-soil">Solo</div>
                        <div className="header-col-acoes-constant">Ações</div>
                    </div>
                    {dadosTemp.map((e: dadosConstants) => (
                        <div key={e.id} className="content-list">
                            <div className="result-col-type">{typeTranslation[e.type]}</div>
                            <div className="result-col-value">{e.value}</div>
                            <div className="result-col-reference">{e.reference}</div>
                            <div className="result-col-climate">{e.climate ? e.climate : "Não informado"}</div>
                            <div className="result-col-biome">{e.biome ? e.biome : "Não informado"}</div>
                            <div className="result-col-country">{e.country}</div>
                            <div className="result-col-irrigation">{irrigationTranslation[e.irrigation] ? irrigationTranslation[e.irrigation] : "Não informado"}</div>
                            <div className="result-col-cultivationSystem">{cultivationSystemTranslation[e.cultivationSystem] ? cultivationSystemTranslation[e.cultivationSystem] : "Não informado"}</div>
                            <div className="result-col-soil">{soilTranslation[e.soil] ? soilTranslation[e.soil] : "Não informado"}</div>
                            <div className="result-col-acoes-constant">
                                <Image src={"/eye.svg"} alt="visualizar" width={24} height={24} />
                                <Image src={"/pencil.svg"} alt="Editar" width={24} height={24} />
                                <Image src={"/delete.svg"} alt="excluir" width={24} height={24} onClick={() => handleDelete(e.id, 'constant')} />
                            </div>
                        </div>
                    ))}
                </div>
